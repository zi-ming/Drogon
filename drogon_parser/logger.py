...
from drogon_parser.settings import SYS_LOG_PATH
from drogon_parser.settings import TASK_LOG_PATH
...
class Logger(object):
    def __init__(self, logger_name, log_file_path=TASK_LOG_PATH, is_rotating_file=False,
        ...
        if log_file_path:
            path = os.path.join(log_file_path, '{}_parser.log'.format(logger_name))
            if is_rotating_file:
                if not max_bytes:
...
sys_logger = Logger('DROGON_PARSER', log_file_path=SYS_LOG_PATH, is_rotating_file=True).logger
